...
const RATING_STYLE: Record<string, { badge: string; bar: string }> = {
  ...
  Pending: { badge: "text-white/30 bg-white/5 border-white/10", bar: "bg-white/20" },
};
...
export default function CandidateDetailPage({ params }: { params: { candidateId: string } }) {
  ...
  if (loading) return (
    <div className="min-h-full bg-[#07070d] flex items-center justify-center">
  ...
  if (error || !c) return (
    <div className="min-h-full bg-[#07070d] flex items-center justify-center">
      <div className="text-center">
        <p className="text-white/40 text-sm mb-3">{error || "Candidate not found"}</p>
      ...
      <div className="px-7 py-6 max-w-6xl">
        ...
        <div className="flex items-start gap-5 mb-6">
          ...
          <div className="flex-1">
            ...
            <p className="text-white/40 text-sm mt-1">{c.applied_role || "Role not set"} {c.total_experience_years ? `· ${c.total_experience_years} yrs` : ""}</p>
            <div className="flex gap-4 mt-2 flex-wrap">
              {c.email && <span className="text-white/25 text-xs">✉ {c.email}</span>}
              {c.phone && <span className="text-white/25 text-xs">📱 {c.phone}</span>}
              {c.location && <span className="text-white/25 text-xs">📍 {c.location}</span>}
              {latestSession && <span className="text-white/25 text-xs">🕐 {latestSession.scheduled_at ? new Date(latestSession.scheduled_at).toLocaleString("en-IN", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" }) : "Unscheduled"}</span>}
            </div>
          </div>
        </div>
          ...
          {(["report", "transcript", "recording"] as Tab[]).map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-4 py-2.5 text-sm font-medium capitalize transition-all border-b-2 -mb-px ${
                activeTab === tab ? "text-violet-300 border-violet-500" : "text-white/25 border-transparent hover:text-white/50"
              }`}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
            <div className="lg:col-span-2 space-y-5">
              ...
              {c.summary && (
                <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-5">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h2 className="text-white/60 text-sm font-medium">Candidate Summary</h2>
                  </div>
              ...
              {skillEvals.length > 0 ? (
                        ...
                        <div key={s.skill}>
                          <div className="flex items-center justify-between mb-1.5">
                            <div className="flex items-center gap-3">
                              ...
                              <span className="text-white/20 text-xs">{s.questions_asked} questions</span>
                            </div>
                            {s.score && <span className="text-white/40 text-sm font-mono">{s.score}/100</span>}
                          </div>
                          {s.score && (
                          ...
                          {s.ai_notes && <p className="text-white/25 text-xs leading-relaxed">{s.ai_notes}</p>}
                        </div>
                      );
                    })}
                  </div>
                ...
                <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-5">
                  <h2 className="text-white/60 text-sm font-medium mb-4">Selected Skills for Interview</h2>
                  ...
                  <div className="space-y-2">
                    {violations.map((v: any, i: number) => (
                      <div key={i} className="flex items-center justify-between">
                        <span className="text-white/40 text-sm">{v.type?.replace(/_/g, " ")}</span>
                        <div className="flex items-center gap-3">
                          {v.timestamp_seconds && <span className="text-white/20 text-xs">at {Math.floor(v.timestamp_seconds / 60)}:{String(Math.floor(v.timestamp_seconds % 60)).padStart(2, "0")}</span>}
              ...
              {/* No interview yet */}
              {!latestSession && (
                <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-8 text-center">
                  <p className="text-white/25 text-sm mb-3">No interview conducted yet</p>
            ...
            <div className="space-y-4">
                  ...
                  <div className="mt-4 space-y-2">
                    {skillEvals.map((s: any) => (
                      <div key={s.skill} className="flex items-center gap-2">
                        <span className="text-white/30 text-xs w-20 shrink-0 truncate">{s.skill}</span>
              ...
              <div className="space-y-2">
                ...
                <Link href={`/dashboard/schedule?candidate=${c.id}`}
                  className="block w-full py-3 bg-white/[0.04] hover:bg-white/[0.07] border border-white/[0.07] text-white/40 hover:text-white/60 font-medium rounded-xl text-sm transition-colors text-center">
                  📅 Schedule Interview
                </Link>
              </div>
            </div>
        ...
        {activeTab === "transcript" && (
          <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl overflow-hidden">
            <div className="px-5 py-4 border-b border-white/[0.05] flex items-center justify-between">
              <h2 className="text-white/60 text-sm font-medium">Interview Transcript</h2>
              <span className="text-white/20 text-xs">{transcript.length} exchanges</span>
            </div>
            <div className="p-5 space-y-4 max-h-[600px] overflow-y-auto">
              {transcript.length === 0 ? (
                <p className="text-white/20 text-sm text-center py-8">No transcript available yet</p>
                ...
                <div key={i} className={`flex gap-4 ${entry.speaker === "candidate" ? "flex-row-reverse" : ""}`}>
                  ...
                  <div className={`max-w-[75%] flex flex-col gap-1 ${entry.speaker === "candidate" ? "items-end" : "items-start"}`}>
                    ...
                    {entry.timestamp_seconds != null && (
                      <span className="text-white/15 text-xs px-1">
        ...
        {activeTab === "recording" && (
          <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl overflow-hidden">
            <div className="aspect-video bg-[#070710] flex items-center justify-center">
              {latestSession?.recording_url ? (
                <video src={latestSession.recording_url} controls className="w-full h-full" />
              ) : (
                <div className="text-center">
                  <div className="w-12 h-12 rounded-full bg-white/[0.04] border border-white/10 flex items-center justify-center mx-auto mb-3">
                    <svg className="w-6 h-6 text-white/20" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M15 10l4.553-2.069A1 1 0 0121 8.87v6.26a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z" /></svg>
                  </div>
                  <p className="text-white/25 text-sm">Recording not available yet</p>
                  <p className="text-white/15 text-xs mt-1">Available after interview is completed</p>
                </div>
              )}
            </div>
          </div>
