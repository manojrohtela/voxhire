...
export default function SchedulePage() {
  ...
  // Filter candidates without completed interviews
    ...
    <div className="min-h-full bg-[#07070d]">
      <div className="sticky top-0 z-20 px-7 py-4 border-b border-white/[0.05] bg-[#07070d]/90 backdrop-blur flex items-center justify-between">
        <div>
          ...
          <p className="text-white/25 text-xs mt-0.5">Pick a candidate, date, and time</p>
        </div>
        ...
        <div className="lg:col-span-2 space-y-5">
          ...
          {step === "done" && newInterview ? (
            <div className="bg-[#0c0c14] border border-emerald-500/20 rounded-2xl p-8 text-center">
              ...
              <p className="text-white/40 text-sm mb-6">{newInterview.candidateName} · {duration} min</p>
              <div className="bg-[#07070d] border border-white/[0.07] rounded-xl px-4 py-3 flex items-center gap-3 mb-4 text-left">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-white/70 text-sm font-mono truncate">{newInterview.interview_link}</p>
                </div>
                <button onClick={() => { navigator.clipboard.writeText(newInterview.interview_link); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
                  className={`shrink-0 px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${
                    copied ? "bg-emerald-500/20 text-emerald-400 border border-emerald-500/20" : "bg-white/[0.05] text-white/40 border border-white/[0.07]"
                  }`}>
                  {copied ? "Copied!" : "Copy"}
                </button>
              </div>
              <button onClick={() => { setStep("pick"); setSelectedCandidateId(null); setSelectedDay(today.getDate()); setSelectedSlot(null); setNewInterview(null); }}
                className="px-6 py-2.5 bg-white/[0.05] hover:bg-white/10 border border-white/[0.08] text-white/50 rounded-xl text-sm transition-colors">
                Schedule Another
              </button>
            </div>
            ...
            <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-6 space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button onClick={() => setStep("pick")} className="flex-1 py-3 border border-white/[0.08] text-white/40 rounded-xl text-sm transition-colors">← Back</button>
            ...
            <>
              ...
              <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-5">
                ...
                {candidatesLoading ? (
                  ...
                  <div className="text-center py-6">
                    <p className="text-white/20 text-sm">No candidates without interviews</p>
                    ...
                    {pendingCandidates.map((c) => (
                      <button key={c.id} onClick={() => setSelectedCandidateId(c.id)}
                        className={`text-left px-4 py-3 rounded-xl border transition-all ${
                          selectedCandidateId === c.id ? "bg-violet-500/10 border-violet-500/30" : "bg-[#0a0a12] border-white/[0.06] hover:border-white/[0.12]"
                        }`}>
                        <div className="flex items-center gap-3">
                          ...
                          <div className="min-w-0">
                            ...
                            <p className="text-white/30 text-xs truncate">{c.applied_role || c.email}</p>
                          </div>
              ...
              <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-5">
                ...
                <div className="grid grid-cols-7 mb-2">
                  {DAY_NAMES.map(d => <div key={d} className="text-center text-white/20 text-xs py-1">{d}</div>)}
                </div>
                    ...
                    return (
                      <button key={day} disabled={isPast || isWeekend} onClick={() => { setSelectedDay(day); setSelectedSlot(null); }}
                        className={`aspect-square rounded-lg text-sm font-medium transition-all flex items-center justify-center ${
                          ...
                          : isPast || isWeekend ? "text-white/[0.12] cursor-not-allowed"
              ...
              {selectedDay && (
                <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-white/70 text-sm font-medium">Select Time</h2>
              ...
              {selectedSlot && (
                <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl p-5">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="text-white/70 text-sm font-medium">Duration</h2>
                  </div>
                  <div className="flex gap-2">
                    {DURATIONS.map((d) => (
                      <button key={d} onClick={() => setDuration(d)}
                        className={`flex-1 py-2.5 rounded-xl text-sm font-medium border transition-all ${
                          duration === d ? "bg-violet-500/15 border-violet-500/30 text-violet-300" : "border-white/[0.07] text-white/30 hover:text-white/50"
                        }`}>
                        {d}m
                      </button>
                    ))}
          ...
          <div className="bg-[#0c0c14] border border-white/[0.06] rounded-2xl overflow-hidden">
            ...
            <div className="divide-y divide-white/[0.04]">
              {upcomingInterviews.length === 0 ? (
                <p className="px-5 py-6 text-white/20 text-xs text-center">No upcoming interviews</p>
              ) : upcomingInterviews.map((s: any) => (
                <div key={s.id} className="px-5 py-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <p className="text-white/70 text-sm font-medium truncate">{s.candidate_id}</p>
                    {s.scheduled_at && (
                  ...
                  {s.interview_link && (
                    <div className="mt-2 flex items-center gap-2 bg-white/[0.03] border border-white/[0.05] rounded-lg px-2.5 py-1.5">
                      <span className="text-white/25 text-xs font-mono truncate">{s.interview_link.replace("https://", "")}</span>
                    </div>
                  )}
                </div>
              ))}
